...
for line in show_vlan:
    if line.startswith("VLAN") or line.startswith("-----") or line.startswith("    "):
        continue
    line = line.split()
    vlan_list.append((line[0],line[1]))
...
for line in show_arp:
    line = line.split()
    ip_mac_list.append((line[1], line[3]))

pprint("IP_MAC LIST:")
pprint(ip_mac_list)
pprint ("-" * 30)

for ip in ip_mac_list:
    if ip[0] == "10.220.88.1":
        pprint("Default gateway IP/Mac is ")
        pprint(ip)
    elif ip[0] == "10.220.88.30":
        pprint("Arista3 IP/Mac is ")
        pprint(ip)
...
for line in lldp_neighbors:
    if line.startswith("System Name"):
        ...
        line = line.split()
        port_id = line[2]
    if port_id == True and system_name == True:
        print("exiting...")
        break
